...
SECTIONS = (NS_TYPE, NS_VNF_TYPE, NS_VL_TYPE, NS_PNF_TYPE, NS_NFP_TYPE, NS_VNFFG_TYPE) = \
    ('tosca.nodes.nfv.NS', 'tosca.nodes.nfv.VNF', 'tosca.nodes.nfv.NsVirtualLink', 'tosca.nodes.nfv.PNF', 'tosca.nodes.nfv.NFP', 'tosca.nodes.nfv.VNFFG')
...
class EtsiNsdInfoModel(BaseInfoModel):
    ...
    def parseModel(self, tosca):
        ...
        self.vls = self.get_all_vl(nodeTemplates, types)
        ...
        self.nested_ns = self.get_all_nested_ns(nodeTemplates, types)
    ...
    def _get_all_vnf(self, nodeTemplates, node_types):
        ...
        for node in nodeTemplates:
            if self.isNodeTypeX(node, node_types, NS_VNF_TYPE):
                ...
                vnf['dependencies'] = self.get_networks(node, node_types)
                vnf['networks'] = self.get_networks(node, node_types)
                vnfs.append(vnf)
        return vnfs
    ...
    def _get_all_pnf(self, nodeTemplates, node_types):
        ...
        for node in nodeTemplates:
            if self.isNodeTypeX(node, node_types, NS_PNF_TYPE):
                ...
                pnf['networks'] = self.get_networks(node, node_types)
                pnfs.append(pnf)
        return pnfs
    ...
    def get_all_nested_ns(self, nodes, node_types):
        nss = []
        for node in nodes:
    ...
    def get_networks(self, node, node_types):
        rets = []
