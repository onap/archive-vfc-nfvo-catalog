...
import functools
import logging
from catalog.pub.utils.toscaparser.const import NS_METADATA, PNF_METADATA, VNF_SECTIONS, PNF_SECTIONS, VL_SECTIONS
...
SDC_SERVICE_METADATA = (SRV_UUID, SRV_INVARIANTUUID, SRV_NAME) = (
...
SDC_PNF_METADATA = (PNF_UUID, PNF_INVARIANTUUID, PNF_NAME, PNF_DESCRIPTION, PNF_VERSION) = \
    ("UUID", "invariantUUID", "name", "description", "version")
SDC_PNF_SECTIONS = (PNF_ID, PNF_METADATA, PNF_PROPERTIES, PNF_DESCRIPTION) = \
    ("name", "metadata", "properties", "description")
...
class SdcServiceModel(BaseInfoModel):
    ...
    def _buildServiceMetadata(self, tosca):
        """ SDC service Meta Format
         ...
         namingPolicy: ''
        """
        metadata_temp = self.buidMetadata(tosca)
        self.setTargetValues(self.metadata, NS_METADATA, metadata_temp, SDC_SERVICE_METADATA)
    ...
    def _get_all_pnf(self, nodeTemplates, node_types):
        ...
        for node in nodeTemplates:
            if self.isNodeTypeX(node, node_types, PNF_TYPE):
                ...
                self.setTargetValues(pnf['properties'], PNF_METADATA, node['metadata'], SDC_PNF_METADATA)
