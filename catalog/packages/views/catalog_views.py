...
from catalog.packages.biz import vnf_package, nspackage
...
def nfpackages_rc(request, *args, **kwargs):
    ...
    if request.method == 'GET':
        ...
        vnf_package.NfDistributeThread(
...
def nf_rd_csar(request, *args, **kwargs):
    ...
    if request.method == 'GET':
        ret = vnf_package.nf_get_csar(csar_id)
        normal_status = status.HTTP_200_OK
        response_serializer = NfPackageSerializer(data=ret[1])

    elif request.method == 'DELETE':
        job_id = str(uuid.uuid4())
        vnf_package.NfPkgDeleteThread(csar_id, job_id).start()
        ret = [0, {"jobId": job_id}]
        normal_status = status.HTTP_202_ACCEPTED
        response_serializer = PostJobResponseSerializer(data=ret[1])
...
def vnf_model_parser(request, *args, **kwargs):
    ...
    logger.debug(
        ...
        csar_id,
        inputs)
    ret = vnf_package.parse_vnfd(csar_id, inputs)
    logger.info("Leave %s, Return value is %s", fun_name(), ret)
    if ret[0] != 0:
        return Response(
            data={
