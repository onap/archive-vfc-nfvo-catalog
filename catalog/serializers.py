...
from catalog.pub.utils.toscaparser.nsdmodel import EtsiVnfdInfoModel
...
class JobResponseHistoryListSerializer(serializers.Serializer):
    ...
    progress = serializers.CharField(help_text="progress", required=False)
    statusDescription = serializers.CharField(help_text="statusDescription", required=False)
...
class JobResponseDescriptorSerializer(serializers.Serializer):
    ...
    errorCode = serializers.CharField(help_text="errcode", required=False)
    responseId = serializers.CharField(help_text="responseId", required=False)
    responseHistoryList = JobResponseHistoryListSerializer(many=True, help_text="responseHistoryList", required=False)

class JobRequestSerializer(serializers.Serializer):
    jobId = serializers.CharField(help_text="this field is generated from a query_serializer", required=False)
    responseDescriptor = JobResponseDescriptorSerializer(help_text="this one too!", required=False)
...
class NsPackageSerializer(serializers.Serializer):
    csarId = serializers.CharField(help_text="csarId", required=True)
    package_info = NsPackageInfoSerializer(help_text="package_info", required=True)
...
class NfPackageDistributeRequestSerializer(serializers.Serializer):
    ...
    vim_ids = serializers.ListField(help_text="vim_ids", child=serializers.CharField(), required=False)
...
class NfPackageSerializer(serializers.Serializer):
    csarId = serializers.CharField(help_text="csarId", required=True)
    packageInfo = NfPackageInfoSerializer(help_text="packageInfo", required=True)
